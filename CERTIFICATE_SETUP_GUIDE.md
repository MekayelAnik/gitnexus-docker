# Secure TLS Setup Guide for GitNexus MCP (Host Server vs Client Machine)
...
> **Note:** This Docker image does NOT auto-generate self-signed certificates. You must provide your own TLS certificates.
...
- Use a placeholder API key in docs and scripts, for example:
  - `gnx_REPLACE_WITH_A_LONG_RANDOM_SECRET`
...
```bash
docker rm -f gitnexus-mcp 2>/dev/null || true
...
curl -v --cacert /path/to/mcp-ca.crt \
  -H "Authorization: Bearer gnx_REPLACE_WITH_A_LONG_RANDOM_SECRET" \
  https://103.102.201.202:8010/healthz
```
